        ...
        for _filter, fields in filters.items():
            ...
            for _field, value in fields.items():
                ...
                if _field in ["countryName", "project.location.countryName"]:
                    ...
                    # Handle special case for GWP intensity
                    if _field == "gwpIntensity":
                        if _filter == "gt":
                            # Use aggregation pipeline for $expr to filter by calculated field
                            query = query.aggregate([
                                {
                                    "$match": {
                                        "$expr": {
                                            "$gt": [
                                                {"$divide": ["$project.results.gwp.total",
                                                             "$project.projectInfo.grossFloorArea.value"]},
                                                value
                                            ]
                                        }
                                    }
    ...
    if sort_by:
        for sort_direction in ["asc", "dsc"]:
            ...
            if not field:
                ...
                if field == "gwpIntensity":
                    # Use aggregation pipeline to sort by calculated field
                    pipeline = [
                        {
                            "$addFields": {
                                "gwpIntensity": {
                        ...
                        {
                            "$sort": {"gwpIntensity": 1 if sort_direction == "asc" else -1}
                        }
                    ]
